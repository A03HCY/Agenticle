...
- **灵活的通信模式**: 使用 `broadcast`、`manager_delegation`、`round_robin`、`voting` 或 `competition` 等模式控制智能体在群组内的交互方式。
...
- `mode`:
    ...
    - `'competition'`: 所有智能体接收相同的输入并并行运行以解决同一任务。然后，一个 `optimizer` (优化器) 智能体会评估所有结果并选出最佳方案。
